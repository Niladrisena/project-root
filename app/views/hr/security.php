<div class="max-w-3xl mx-auto pb-12 px-4 md:px-0">
    ...
    <div class="mb-8 flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">Manage your HR Command Center access and cryptographic credentials.</p>
            </div>
        </div>
    ...
    <form action="<?= base_url('/hr/security') ?>" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
        
        <div class="p-8 space-y-6">
            <h3 class="text-sm font-black text-gray-900 uppercase tracking-widest border-b border-gray-100 pb-3 mb-6">Change HR Dashboard Password</h3>
            ...
            <div>
                ...
                <input type="password" name="current_password" required placeholder="Enter your current password" class="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-indigo-900 outline-none transition">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                <div>
                    ...
                    <input type="password" name="new_password" required placeholder="Minimum 8 characters" class="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-indigo-900 outline-none transition">
                </div>
                <div>
                    ...
                    <input type="password" name="confirm_password" required placeholder="Retype new password" class="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-indigo-900 outline-none transition">
                </div>
            </div>
            
            <div class="mt-4 p-4 bg-indigo-50 rounded-xl border border-indigo-100 flex gap-3 items-start">
                ...
                <p class="text-xs text-indigo-800 font-medium leading-relaxed">
                    <span class="font-bold uppercase tracking-wider">Security Tip:</span> As a Human Resources administrator handling sensitive data, ensure you use a mix of uppercase, lowercase, numbers, and symbols.
                </p>
            </div>
        </div>

        <div class="bg-gray-50 px-8 py-5 border-t border-gray-100 flex justify-end">
            <button type="submit" class="px-8 py-3.5 bg-indigo-900 hover:bg-indigo-800 text-white font-black rounded-xl shadow-md transition flex items-center gap-2">
