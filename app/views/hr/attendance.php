<div class="max-w-7xl mx-auto pb-12 px-4 md:px-0">
    <div class="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-8">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 bg-rose-900 text-white rounded-xl flex items-center justify-center">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            </div>
            <div>
                <h1 class="text-2xl font-black text-gray-900 tracking-tight">Global Attendance Monitor</h1>
                <p class="text-sm text-gray-500 mt-0.5">Manage your daily timesheet and monitor corporate personnel presence.</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-1">
            <div class="bg-gradient-to-br from-rose-800 to-rose-900 rounded-2xl shadow-lg p-8 text-center text-white relative overflow-hidden">
                <h3 class="font-black text-rose-200 uppercase tracking-widest text-xs mb-4 relative z-10">Current Time</h3>
                <h1 class="text-5xl font-black tracking-tight mb-2 relative z-10" id="live-clock">00:00:00</h1>
                <p class="text-rose-300 text-sm font-medium mb-8 relative z-10"><?= date('l, F j, Y') ?></p>

                <form action="<?= base_url('/hr/clock_event') ?>" method="POST" class="relative z-10">
                    <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                    <?php if(!$my_attendance): ?>
                        <button type="submit" class="w-full py-4 bg-white text-rose-900 font-black rounded-xl shadow-xl hover:scale-105 transform transition duration-200">CLOCK IN NOW</button>
                    <?php elseif(!$my_attendance['clock_out']): ?>
                        <div class="mb-4 text-rose-200 text-sm font-bold bg-rose-800 py-2 rounded-lg border border-rose-700">Clocked In: <?= date('h:i A', strtotime($my_attendance['clock_in'])) ?></div>
                    ...
                    <?php else: ?>
                        <div class="p-4 bg-rose-800 rounded-xl border border-rose-700">
                            <span class="block text-rose-200 font-bold mb-1">Shift Completed</span>
                            <span class="text-xs text-rose-300">Out at <?= date('h:i A', strtotime($my_attendance['clock_out'])) ?></span>
                        </div>
                    <?php endif; ?>
                </form>
            </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden h-full">
                <div class="p-5 border-b border-gray-100 bg-gray-50"><h3 class="font-bold text-gray-900">Today's Corporate Roster</h3></div>
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <?php if(empty($team_attendance)): ?>
                                <tr><td colspan="4" class="px-6 py-12 text-center text-gray-500">No personnel have clocked in yet today.</td></tr>
                            <?php else: ?>
                                <?php foreach($team_attendance as $t): ?>
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-6 py-4">
                                            <?php if($t['status'] == 'late') echo '<span class="px-2.5 py-1 bg-red-100 text-red-700 text-[10px] font-black uppercase rounded">Late</span>';
                                            else echo '<span class="px-2.5 py-1 bg-green-100 text-green-700 text-[10px] font-black uppercase rounded">Present</span>'; ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
